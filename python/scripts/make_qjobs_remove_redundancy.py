...
REMOVE_REDUN = 'python /users/rg/mmariotti/Scripts/selenoprofiles_remove_redundancy.py'
...
def main():
    ...
    for genome in genomeslist:
        ...
        with open(jobfilename, 'w') as tjf:
            ...
            tjf.write('PATH=$PATH:/users/rg/agrimaldi/usr/bin:/users/rg/agrimaldi/Code/python/python/scripts:/users/rg/mmariotti/bin:/users/rg/mmariotti/Scripts\n')
            tjf.write('echo "host : $HOSTNAME"\n')
            tjf.write('(' + ' '.join(( options.sppath,
                                 ' '.join(['-'+o for o in options.sp_args.split(',')]),
                                 '-i', os.path.abspath(options.genomesfolder) +'/'+ genome,
                                 '-temp', options.tempfolder + ')',
                                 '> ' + os.path.abspath(options.outputfolder) + '/' + genome+'.nr',
				 '2> ' + os.path.abspath(options.outputfolder) + '/' + genome+'.nr.LOG'
                                 '\n')))
